...
Ordered by: & Redacted & Received: & \data{date_received} \\
...
\\ \hline
\begin{dataiter}{variant}
\data{variant:gene_symbol} & \data{variant:hgvsc} & \data{variant:hgvsp} & \data{variant:zygosity} & \data{variant:interpretation}\\ \hline
\end{dataiter}
\end{tabular}
...
HEREDITARY BREAST AND OVARIAN CANCER \data{tested_genes:num} gene panel, HOXB13 c.251G>A (p.Gly84Glu) variant sequencing, and MLPA for a subset of genes 
\begin{flushright}
Electronically signed by [Not yet signed] on [Date] at [Time]
\end{flushright}

\section*{Test Details}
Genes analyzed: \data{tested_genes}
...
The following transcripts are used in this analysis: \begin{dataiter}{tested_genes}
\data{tested_genes:gene_symbol} (\data{tested_genes:refseq_mrna}),
\end{dataiter}
